...
SELECT
    ...
    #COUNT(equipamento_id) as total_equipamento,
...
FROM equipamento AS E
WHERE
NOT EXISTS (SELECT * FROM aluguel AS R WHERE R.equipamento_id = E.id);
...
SELECT
     e.nome,
count(a.equipamento_id) as total     
FROM equipamento e
inner join aluguel a on a.equipamento_id = e.id
group by e.nome
having count(a.equipamento_id) > 1
